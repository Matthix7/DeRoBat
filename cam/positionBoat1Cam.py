...
def getCap(corners1, ids1):
    ...
    angle = np.arctan2(abs(yAr4 - yAr5), abs(xAr4 - xAr5))

    angle = angle + math.pi/2

    """
    if yAr5 <= yAr4 and xAr5 >= xAr4: #part 1
        print("1")        
    elif yAr5 >= yAr4 and xAr5 >= xAr4: #part 2 
        print("2")
    elif yAr5 >= yAr4 and xAr5 <= xAr4: #part 3 
        print("3")
    elif yAr5 <= yAr4 and xAr5 <= xAr4: #part 4
        print("4")
    """
    return (angle)
...
def init():
    ...
    cap1.open('http://root:1234@169.254.69.55/mjpg/video.mjpg')
    
    DIM, K, D = calibration.defineCalibrationParameters()
    
    return cap1, DIM, K, D

def run(cap1, DIM, K, D):
    ...
    cv2.imshow("Camera 1", resizeFrame(frame1))
...
if __name__ == "__main__":
    ...
    cap1, DIM, K, D = init() # initilalisation à ne lancer que 1 fois
    file = open('testfile.txt', 'w')
    while(True):
        xBoat, yBoat,theta = run(cap1, DIM, K, D)# à mettre dans une bouvle while(true)
